...
function slide(chan) {
  while(true) {
    var receivers = chan[/* receivers */3];
    var buf = chan[/* buf */1];
    if (Queue.is_empty(receivers) || Queue.is_empty(buf)) {
      ...
      var match$2 = match[1][/* ok */0];
      if (match$2) {
        Queue.pop(buf);
        continue ;
      } else {
        var match$3 = match$1[/* ok */0];
        if (match$3) {
          Queue.pop(receivers);
          continue ;
        } else {
          ...
          receiver[/* ok */0] = true;
          sender[/* ok */0] = true;
          sender[/* resolve */1](item);
          return receiver[/* resolve */1](item);
        }
      }
    }
  };
  ...
  return new Promise((function (resolve, reject) {
                ...
                if (match) {
                  for(var i = 0 ,i_finish = arr.length - 1 | 0; i <= i_finish; ++i){
                    ...
                    var buf = chan[/* buf */1];
                    var cap = chan[/* cap */0];
                    Promise.resolve(Curry._1(chan[/* transformer */2], item)).then((function(cap,buf,chan){
...
function put(chan, item) {
  return deliver(/* array */[chan], item);
}

function is_completed(param) {
  var status = param[/* status */4];
  if (status === /* Closed */0) {
    return Queue.length(param[/* buf */1]) === 0;
  } else {
    return false;
  }
}
...
function oneof(arr) {
  return new Promise((function (resolve, reject) {
                ...
                for(var i = 0 ,i_finish = arr.length - 1 | 0; i <= i_finish; ++i){
                  var chan = Caml_array.caml_array_get(arr, i);
                  Queue.push(receiver, chan[/* receivers */3]);
                  slide(chan);
                }
                return /* () */0;
              }));
